...
def show_toast():
    ...
    # 알림 텍스트 레이블
    label = ctk.CTkLabel(alert_window, text="스트레칭 하세요~~", font=("Roboto", 18,"bold"), text_color="white", bg_color=root.cget('bg'))
    label.pack(pady=20)

    # 확인 버튼을 root의 기본 색상으로 설정
    ok_button = ctk.CTkButton(alert_window, text="확인", command=alert_window.destroy, width=120, height=40,
                              corner_radius=10,fg_color="#1e90ff", hover_color="#4682b4")
...
def countdown(remaining):
    ...
    if remaining >= 0:
        ...
        countdown_label.configure(text=f"허리부서지기까지.. {minutes}분 {seconds}초")
...
def start_gui():
    ...
    # 다음 스트레칭까지 남은 시간 레이블
    next_alert_label = ctk.CTkLabel(root, text="허리를 지켜요!", font=("Roboto", 25))
